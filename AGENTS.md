...
### 2026-09-22 — cline live /models + OpenRouter 429 retry (model-list reliability)
- **Trigger:** Open WebUI offered cline models that failed with "Provider cline rejected the request (HTTP 404)" and OpenRouter free models that failed with "Provider openrouter nicht erreichbar, kein Fallback/Queue konfiguriert" (429). Both are upstream-catalog/rate-limit issues surfaced through `/v1/models`.
- **Cline root cause:** `providers/cline.py` docstring claimed `GET /models` returns 404, so `get_models()` always read `pricing_overrides_cline.json` (489 IDs generated from Cline's OSS client catalog). Cline now exposes a public `GET https://api.cline.bot/api/v1/models` returning the 444 IDs the account can actually serve; the override list carried stale IDs with different casing (`Qwen/Qwen3-235B-A22B` vs live `qwen/qwen3-235b-a22b`), which the API rejects with 404. Confirmed: `cline/Qwen/Qwen3-235B-A22B` → 404, `cline/qwen/qwen3-235b-a22b` → 200.
  - **Fix:** `get_models()` queries the live endpoint first (in-process 600 s TTL cache) and falls back to the override file only on failure. Tests updated (`test_get_models_uses_live_api`, `test_get_models_falls_back_to_override_when_api_fails`).
- **OpenRouter root cause:** free-tier models share an upstream pool and return transient 429s. `_RETRYABLE_4XX` (§3.11) routes a 429 to the configured fallback, but most requests have no fallback, so the user saw a hard "nicht erreichbar" error.
  - **Fix:** `OpenRouterClient.create_message()` retries a 429 up to 3 attempts with exponential backoff (2 s/4 s), honouring `Retry-After` (capped 30 s). After exhaustion it re-raises so dispatch-level fallback still applies.
- **Commits:** `fe9c3ac` (code+tests) → `Merge` `ac22fdb` → `main`, pushed.
- **Deploy (oracle-vm):** remote checkout had drifted (`2041bec` + dirty `providers/opencode.py`, `tests/test_opencode_provider.py`, `docker-compose.yml` — the 2026-09-18/09-21 changes that were applied directly). Backed up to `/root/ai-provider-localmods-20260922.bak`, then `sudo git checkout -- .` + `sudo git pull --ff-only origin main` → `ac22fdb`. `sudo docker compose build` + `down`/`up -d` → container `healthy`, `RestartCount=0`, created `2026-09-22T08:44:10Z`.
- **Verified on oracle-vm:** `pytest tests/test_cline_provider.py tests/test_openrouter_provider.py` → 31 passed; full suite → 464 passed, 1 pre-existing failure (`test_opencode_raises_without_api_key`, red on clean main); `/v1/models` → 488 (cline 444 / openrouter 24 / ollama 20), stale uppercase cline IDs gone; chat smoke `cline/qwen/qwen3-235b-a22b`, `cline/minimax/minimax-m2`, `openrouter/liquid/lfm-2.5-2.6b:free` → 200; `/health` → 200.
- **Follow-up:** the Open WebUI side needed a workspace-table resync plus a Redis base-model-cache refresh (and flock to serialize overlapping sync runs); see `wolfini_de_web` AGENTS.md §7 2026-09-22 and hard rule §3.16 there.
